...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\" The core of a wand is a magical substance placed within the length of wood, usually extracted from a magical creature. The core type is important and influences the wand. lo haré not use Veela hair, as that makes for far too temperamental wands. \\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
...
################################
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=1}] run data merge entity @s {CustomName:"{\"text\":\"≥ Yo haré eso now, thanks\"}",Tags:["_____Return_____","_____click_____","convOption","i"]}
